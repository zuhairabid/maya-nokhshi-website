    ...
    <section className="py-12 md:py-24 px-4 md:px-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-12">
        {products.map((product) => (
          ...
          >
            <div className="aspect-[4/5] bg-stone-100 mb-4 md:mb-8 overflow-hidden relative shadow-sm hover:shadow-xl transition-all duration-500">
              <img 
                src={product.image} 
                alt={product.name.EN}
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button className="bg-white/95 backdrop-blur-sm px-4 md:px-8 py-2 md:py-4 text-[8px] md:text-[9px] uppercase tracking-[0.2em] font-bold text-stone-900 border border-stone-200">
                  Select Options
                </button>
              </div>
            </div>
            <div className="text-center px-2">
              <p className="text-[8px] md:text-[9px] uppercase tracking-widest text-stone-400 mb-1 md:mb-2 font-semibold">{product.category}</p>
              <h3 className="text-stone-800 text-[11px] md:text-sm font-medium mb-1 md:mb-2 tracking-tight group-hover:text-[#C5A059] transition-colors line-clamp-1">
